...
Wrong — `helperMethod` is not a `@Test`, so it does not belong here:
...
Correct — the name explains what is expected:
...
Wrong — `createUser` is imperative, `userIsCreated` is passive voice,
`userWasRemoved` is past tense:
